...
assert(manifest.healthcheck === undefined, "Manifest must not use singular healthcheck.");
assert(Array.isArray(manifest.healthchecks), "Manifest must use canonical healthchecks[].");
assert(manifest.healthchecks.length === 1, "TypeDB should declare one readiness healthcheck.");
const healthcheckIds = new Set(manifest.healthchecks.map((check) => check?.id));
assert(healthcheckIds.size === manifest.healthchecks.length, "Healthcheck ids must be unique.");
const tcpReadyCheck = manifest.healthchecks[0];
assert(tcpReadyCheck?.id === "typedb-tcp-ready", "TCP healthcheck must have a stable id.");
assert(tcpReadyCheck?.type === "tcp", "Healthcheck must be TCP.");
assert(tcpReadyCheck?.address === "${endpoint.service.bind}:${endpoint.service.port}", "TCP healthcheck must use endpoint selectors.");
assert(tcpReadyCheck?.tcphost === undefined, "TCP healthcheck must not use tcphost alias.");
assert(tcpReadyCheck?.tcpport === undefined, "TCP healthcheck must not use tcpport alias.");
